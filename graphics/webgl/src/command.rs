...
use sourcerenderer_core::graphics::{BindingFrequency, BufferUsage, CommandBuffer, MemoryUsage, PipelineBinding, Scissor, ShaderType, Texture, Viewport};
use wasm_bindgen::JsCast;
use web_sys::{WebGlRenderingContext, WebGlTexture};

use crate::{WebGLBackend, WebGLBuffer, WebGLGraphicsPipeline, WebGLTexture, WebGLTextureShaderResourceView, RawWebGLContext, format_to_internal_gl, address_mode_to_gl, max_filter_to_gl, min_filter_to_gl};
...
impl CommandBuffer<WebGLBackend> for WebGLCommandBuffer {
  ...
  where T: 'static + Send + Sync + Sized + Clone {
    Arc::new(WebGLBuffer::new(&self.context, std::mem::size_of_val(data), MemoryUsage::CpuOnly, usage))
  }
  ...
  fn bind_texture_view(&mut self, frequency: BindingFrequency, binding: u32, texture: &Arc<WebGLTextureShaderResourceView>) {
