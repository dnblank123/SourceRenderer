...
use sourcerenderer_core::graphics::{Buffer, Device, Texture};
use wasm_bindgen::JsCast;
use web_sys::{WebGlRenderingContext, WebGlTexture};

use crate::{RawWebGLContext, WebGLBackend, WebGLBuffer, WebGLComputePipeline, WebGLFence, WebGLGraphicsPipeline, WebGLRenderGraph, WebGLRenderGraphTemplate, WebGLShader, WebGLSurface, WebGLSwapchain, WebGLTexture, WebGLTextureShaderResourceView, format_to_internal_gl};
...
impl WebGLDevice {
  pub fn new(surface: &WebGLSurface) -> Self {
    Self {
      context: Rc::new(RawWebGLContext::new(surface))
    }
  }
}

impl Device<WebGLBackend> for WebGLDevice {
  fn create_buffer(&self, size: usize, memory_usage: sourcerenderer_core::graphics::MemoryUsage, usage: sourcerenderer_core::graphics::BufferUsage, _name: Option<&str>) -> Arc<WebGLBuffer> {
    Arc::new(WebGLBuffer::new(&self.context, size, memory_usage, usage))
  }
  ...
  fn upload_data<T>(&self, data: &[T], memory_usage: sourcerenderer_core::graphics::MemoryUsage, usage: sourcerenderer_core::graphics::BufferUsage) -> Arc<WebGLBuffer> where T: 'static + Send + Sync + Sized + Clone {
    let buffer = Arc::new(WebGLBuffer::new(&self.context, std::mem::size_of_val(data), memory_usage, usage));
    unsafe {
  ...
  fn create_graphics_pipeline(&self, info: &sourcerenderer_core::graphics::GraphicsPipelineInfo<WebGLBackend>, _graph_template: &WebGLRenderGraphTemplate, _pass_name: &str, _subpass_index: u32) -> Arc<WebGLGraphicsPipeline> {
  ...
  fn create_render_graph_template(&self, _info: &sourcerenderer_core::graphics::RenderGraphTemplateInfo) -> Arc<WebGLRenderGraphTemplate> {
    todo!()
  }

  fn create_render_graph(&self,
                        _template: &Arc<WebGLRenderGraphTemplate>,
                        _info: &sourcerenderer_core::graphics::RenderGraphInfo<WebGLBackend>,
                        _swapchain: &Arc<WebGLSwapchain>,
                        _external_resources: Option<&std::collections::HashMap<String, sourcerenderer_core::graphics::ExternalResource<WebGLBackend>>>) -> WebGLRenderGraph {
    todo!()
  }
